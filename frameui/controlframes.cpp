...
LRESULT EditFrame::onMessage(uint32 message, WPARAM wParam, LPARAM lParam) {
  if (message == WM_CTLCOLOREDIT || message == WM_CTLCOLORSTATIC) {
    ...
    if (background) return (LRESULT) background;
    return M_UNHANDLED;
  }
  return M_UNHANDLED;
}
...
RangeSliderFrame::RangeSliderFrame(Frame* parent, int id)
  : WindowFrame(parent)
  , min_(0)
  , max_(100)
  , left_(min_)
  ...
  , hovering_(0)
{
  if (WNDCLASSEX* wcx = createclass("RangeSliderFrmClass")) {
    wcx->hCursor = LoadCursor(NULL, IDC_ARROW);
    wcx->hbrBackground = HBRUSH(COLOR_BTNSHADOW + 1);
...
LRESULT RangeSliderFrame::onMessage(uint32 message, WPARAM wParam, LPARAM lParam) {
  RECT rc;
  switch (message) {
  case WM_PAINT: {
    PAINTSTRUCT ps;
    ...
    //rc.top += 4;
    //rc.bottom -= 4;
    DrawEdge(hDC, &rc, BDR_SUNKENOUTER, BF_ADJUST | BF_RECT);
    //SetBkColor(hDC, GetSysColor(COLOR_BTNSHADOW));
    //ExtTextOut(hDC, 0, 0, ETO_OPAQUE, &rc, NULL, 0, NULL);
    int width = rc.right - rc.left;
    rc.right = rc.left + (right_ - min_) * width / (max_ - min_);
    rc.left = rc.left + (left_ - min_) * width / (max_ - min_);
  ...
  case WM_LBUTTONDOWN: {
    ...
    rc.left += 1;
    rc.right -= 1;
    hover_ = min_ + (GET_X_LPARAM(lParam) - rc.left) * (max_ - min_) / (rc.right - rc.left);
    if (hover_ < min_) hover_ = min_;
    if (hover_ > max_) hover_ = max_;
    if (std::abs(hover_ - left_) < std::abs(hover_ - right_)) {
      drag_ = DRAG_LEFT;
      left_ = hover_;
    } else {
      drag_ = DRAG_RIGHT;
    ...
    invalidate();
    notify(WM_COMMAND, MAKELONG(id(), DRAG_START | drag_), (LPARAM) hWnd);
    SetCapture(hWnd);
    return 0;
  }
  case WM_MOUSEMOVE: {
    ...
    if (hover_ < min_) hover_ = min_;
    if (hover_ > max_) hover_ = max_;
    if (!(wParam & MK_LBUTTON) || !drag_) {
      notify(WM_COMMAND, MAKELONG(id(), HOVER), (LPARAM) hWnd);
    } else {
      if (drag_ & DRAG_LEFT) {
        left_ = hover_;
      } else {
      ...
      invalidate();
      notify(WM_COMMAND, MAKELONG(id(), DRAG_MOVE | drag_), (LPARAM) hWnd);
    }
    return 0;
  }
  case WM_MOUSELEAVE:
    if (hovering_) {
      notify(WM_COMMAND, MAKELONG(id(), LEAVE), (LPARAM) hWnd);
    }
    hovering_ = 0;
    return 0;
  case WM_LBUTTONUP:
    if (drag_) {
      ReleaseCapture();
      notify(WM_COMMAND, MAKELONG(id(), DRAG_END | drag_), (LPARAM) hWnd);
      drag_ = 0;
    }
    return 0;
  default:
